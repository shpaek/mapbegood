<template>
  ...
  export default {
    ...
    mounted() {
  const themeMapDetail = this.$store.state.themeMapDetail;
  if (themeMapDetail) {
    this.mymapdetail = {
      themeMapDto: themeMapDetail.themeMap,
      myplaces: themeMapDetail.myPlaces,
    };
  } else {
    // If themeMapDetail is not in the store, redirect to the main page or handle it accordingly.
    this.$router.push({ name: "thememapdetail" });
  }
},
    methods: {
