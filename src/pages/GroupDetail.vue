        ...
        <v-dialog v-model="groupThemeMapAddDialog" persistent width="500">
          ...
          <v-card>
            ...
            <div class="btn-create-thememap">
              <!-- 테마맵 생성 버튼 -->
              <span @click="CreateGroupThemeMap" class="create-thememap"
                >생성</span
              >
              <span @click="cancelCreateGroupMap" class="cancel-thememap"
                >취소</span
              >
            </div>
            <br />
...
export default {
  ...
  methods: {
    ...
    async CreateGroupThemeMap() {
      ...
      try {
        ...
        const response = await axios.post(
          `${this.backURL}/ourmap/create/${groupId}`,
          groupthemeMapDto,
          {
            headers: {
    ...
    updateGroupMap(themeMapDto) {
      const url =
        `${this.backURL}/ourmap/update/${this.groupId}/` + themeMapDto.id;
